...
import { MapPin, Users, Trash2, Zap, Leaf } from 'lucide-react';
...
export function BaliMap({ regencyStats = {}, loading = false }) {
  ...
  const regencies = [
    ...
    },
    {
      id: "Gianyar",
      name: "Gianyar",
      path: "M325.9,206.6 L327.6,204.6 L332.7,202.2 L334.5,199.3 L340.6,196 L344.2,191.1 L348.7,189.5 L348.8,186.7 L347.8,185 L346.8,185.1 L347.3,184.3 L346.4,181.1 L345.5,180.7 L346.3,179.6 L346.3,176.6 L345.3,175.2 L345.3,172.8 L344.4,172.5 L344.4,171.6 L340.3,172.9 L336.8,171.8 L336.7,170.3 L334.9,169 L336,167.7 L335,167.1 L335,164.9 L333.3,161.7 L334.5,159.6 L333.5,154.4 L334.3,153.1 L331.1,142.9 L332.9,141.2 L335.5,136.4 L335.4,132.8 L337.6,125.6 L336.3,124.4 L337.3,123 L337.4,120.5 L336.4,120.4 L334.9,123 L334.7,120.1 L333.8,119.9 L335.3,117.8 L335.6,115.2 L334.3,111.3 L334.7,108.7 L332.7,108 L330.9,110.7 L330.3,108.4 L328,107.8 L326.6,110 L323.3,109.4 L322.3,104 L320.2,103.1 L315.7,106.1 L312.9,109.5 L313.6,106.7 L313,105 L311.7,103.6 L309.1,103.4 L308,104.4 L308.5,105.4 L307.1,105.9 L307.4,107.8 L306.6,107.9 L308.2,109.2 L308.6,111.3 L308.1,112.3 L306,112.8 L306.2,113.7 L303.6,117.8 L304.5,120 L303.3,121.5 L304,123.1 L302.8,124.6 L302.6,126.4 L301.6,126.4 L300.7,127.8 L301.7,129.9 L301.6,137.1 L302.4,138 L302.4,140.2 L303.4,141 L302.1,146.6 L304.2,150.9 L306.3,153.2 L305.9,155 L307,155.4 L306.3,155.6 L306.2,156.8 L307.1,157.3 L306,160.1 L306.7,160.6 L305.6,161.8 L306.6,164.5 L304.8,165.6 L305,167.6 L302.5,170.3 L302.2,173.5 L303.2,174.4 L302.6,177.3 L303.4,177.6 L303.9,179.7 L305.2,180.5 L306,183.2 L307.1,183.9 L307.1,185.9 L308.6,189.3 L308.7,192.7 L306.3,198.7 L306.5,200.4 L307.2,202 L310.7,205.3 L312.2,209.7 L313.5,210 L314,211.4 L315.2,211.4 L317.1,214.3 L322.1,210.6 L325.9,206.6 Z",
      labelX: 320.4,
      labelY: 163,
      color: "#F3C5D8"
    },
  ...
  return (
    <Card className="flex flex-col items-center">
      {/* Header Mini */}
      <div className="flex items-center justify-between w-full mb-6 border-b border-brand-light pb-4">
        <div>
          <span className="text-xs font-bold text-brand-primary uppercase tracking-wider">Visualisasi Real-time</span>
          <h2 className="text-xl font-bold text-brand-dark mt-0.5 font-display">Peta Bali Bercahaya</h2>
        </div>
        <div className="flex items-center text-xs font-semibold bg-brand-light text-brand-primary px-3 py-1.5 rounded-full border border-brand-primary/5">
          <MapPin className="w-3.5 h-3.5 mr-1" />
          Tap wilayah untuk detail
        </div>
      </div>

      {/* SVG Map Container (Premium White Canvas with Island Drop Shadow) */}
      <div className="relative w-full max-w-lg aspect-[5/3] bg-white border border-brand-light rounded-3xl overflow-hidden p-2.5 shadow-premium mb-6 flex items-center justify-center">
        <svg
          viewBox="0 0 500 300"
        ...
        >
          <defs>
            {/* Drop shadow for the entire island of Bali (snug & premium) */}
            <filter id="islandShadow" x="-10%" y="-10%" width="130%" height="130%">
              <feDropShadow dx="-2" dy="3.5" stdDeviation="4.5" flood-color="#1A331E" flood-opacity="0.13" />
            </filter>
            
            {/* Elegant brand primary glow for active selected regency */}
          ...
          {/* Group wrapping all regencies to cast a cohesive island-wide drop shadow */}
          <g filter="url(#islandShadow)">
            {regencies.map((reg) => {
              const data = regencyStats[reg.id] || { total_kg: 0 };
              const isSelected = selectedRegency === reg.id;
              
              // Hitung rasio kontribusi sampah dibanding maxKg
              const ratio = data.total_kg / maxKg;
              const contributionLevel = Math.max(0.1, ratio * 0.9);

              // Tentukan style warna & opacity
              const fillColor = reg.color;
              const strokeColor = isSelected
                ? '#2d7a4f' // Brand primary green for selected border
                : '#ffffff'; // White borders to separate regencies

              return (
                <g 
                  key={reg.id}
                  onClick={() => setSelectedRegency(reg.id)}
                  className="cursor-pointer group"
                >
                  {/* Glowing Aura Behind Selected Regency */}
                  {isSelected && (
                    <path
                      d={reg.path}
                      fill="none"
                  ...
                  <path
                    ...
                    className="transition-all duration-300 group-hover:opacity-90 group-hover:stroke-brand-primary"
                    style={{
                      filter: isSelected ? 'url(#selectedGlow)' : 'none',
                      transform: isSelected ? 'scale(1.02)' : 'none',
                      transformOrigin: `${reg.labelX}px ${reg.labelY}px`,
                    }}
                  />

                  {/* Regency Text Label (Dark Elegant typography) */}
                  <text
                    x={reg.labelX}
                    y={reg.labelY}
                    className={`text-[8px] font-sans font-extrabold pointer-events-none select-none transition-all duration-300 ${
                      isSelected 
                        ? 'fill-brand-dark scale-110' 
                        : 'fill-brand-dark/75 group-hover:fill-brand-dark'
                    }`}
                    textAnchor="middle"
                  >
                    {reg.name}
                  ...
                  {/* Contribution indicator dot if they have significant trash sorted */}
                  {contributionLevel > 0.4 && !isSelected && (
                    <circle
                      cx={reg.labelX}
                      cy={reg.labelY - 7}
                      r="1.8"
                      className="fill-brand-yellow animate-pulse-slow"
                    />
                  )}
                </g>
              );
      ...
      <div className="w-full bg-[#F9FBF9] border border-brand-light rounded-2xl p-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white p-3 rounded-xl border border-brand-light">
            <div className="flex items-center text-[10px] text-brand-textSecondary font-bold uppercase tracking-wider">
              <Trash2 className="w-3.5 h-3.5 text-brand-primary mr-1 stroke-[2.5px]" />
              Terpilah
            </div>
            <div className="text-lg font-bold text-brand-dark mt-1 font-display">
              {selectedData.total_kg.toLocaleString('id-ID')} <span className="text-xs font-semibold">kg</span>
            </div>
          </div>

          <div className="bg-white p-3 rounded-xl border border-brand-light">
            <div className="flex items-center text-[10px] text-brand-textSecondary font-bold uppercase tracking-wider">
              <Zap className="w-3.5 h-3.5 text-brand-yellow mr-1 shrink-0" />
              Potensi
            </div>
            <div className="text-lg font-bold text-brand-dark mt-1 font-display">
              {selectedData.total_kwh.toLocaleString('id-ID')} <span className="text-xs font-semibold">kWh</span>
            </div>
          </div>

          <div className="bg-white p-3 rounded-xl border border-brand-light">
            <div className="flex items-center text-[10px] text-brand-textSecondary font-bold uppercase tracking-wider">
              <Leaf className="w-3.5 h-3.5 text-brand-primary mr-1 shrink-0" />
              CO₂e Saved
            </div>
            <div className="text-lg font-bold text-brand-dark mt-1 font-display">
              {selectedData.total_co2e.toLocaleString('id-ID')} <span className="text-xs font-semibold">kg</span>
            </div>
          </div>
        </div>
      </div>
