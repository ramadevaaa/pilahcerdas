...
export function ChoroplethMap({ regencyStats = {}, onSelectRegency, selectedRegency }) {
  // Menentukan kabupaten dengan sampah tertinggi untuk menghitung skala warna
  ...
  return (
    <div className="flex flex-col items-center bg-white border border-brand-light rounded-3xl p-6 shadow-premium w-full h-auto">
      {/* Header */}
      <div className="flex items-center justify-between w-full mb-6 border-b border-brand-light pb-4 shrink-0">
        <div>
          <span className="text-xs font-bold text-brand-primary uppercase tracking-wider">Pemetaan Geografis</span>
          <h2 className="text-lg font-bold text-brand-dark mt-0.5 font-display">Peta Distribusi Timbulan Bali</h2>
        </div>
        <div className="text-[10px] font-bold bg-brand-light text-brand-primary px-2.5 py-1 rounded-full border border-brand-primary/5">
          Gradasi Kepadatan Volume
        </div>
      </div>

      {/* SVG Choropleth Map */}
      <div className="relative w-full aspect-[5/3] bg-emerald-50/10 border border-brand-light/60 rounded-3xl overflow-hidden p-3 shadow-inner flex items-center justify-center">
        <svg
          viewBox="0 0 500 300"
        ...
        >
          <defs>
            <filter id="islandShadow" x="-10%" y="-10%" width="130%" height="130%">
              <feDropShadow dx="-1.5" dy="3.0" stdDeviation="3.5" flood-color="#113F1F" flood-opacity="0.08" />
            </filter>
          ...
          <g filter="url(#islandShadow)">
            {regencies.map((reg) => {
              ...
              // Hitung porsi kontribusi untuk menentukan warna gradasi
              const ratio = data.total_kg / maxKg;
              
              // Skala warna Choropleth (Emerald/Green Gradient)
              // Jika kg = 0: abu-abu hijau tipis (#E2ECE5)
              // Jika tinggi: gradasi hijau tua
              let fillColor = '#E2ECE5';
              if (data.total_kg > 0) {
                if (ratio < 0.15) fillColor = '#C2E5D0';      // Hijau sangat muda
                else if (ratio < 0.40) fillColor = '#8CD1A4'; // Hijau muda
                else if (ratio < 0.70) fillColor = '#4CAD73'; // Hijau sedang
                else fillColor = '#20633C';                   // Hijau tua pekat
              }

              const strokeColor = isSelected ? '#F5A623' : '#FFFFFF'; // Kuning emas untuk perbatasan aktif

              return (
                <g 
                  key={reg.id}
                  onClick={() => onSelectRegency(isSelected ? '' : reg.id)}
                ...
                >
                  ...
                  <text
                    ...
                    className={`text-[8px] font-sans font-extrabold pointer-events-none select-none transition-all duration-300 ${
                      isSelected 
                        ? 'fill-brand-yellow font-black scale-110' 
                        : data.total_kg > maxKg * 0.4 
                          ? 'fill-white' 
                          : 'fill-brand-dark/80 group-hover:fill-brand-dark'
                    }`}
                    textAnchor="middle"
                  >
                    {reg.name}
                  ...
                  {isSelected && (
                    <circle
                      ...
                      cy={reg.labelY - 7}
                      r="2.5"
                      className="fill-brand-yellow animate-ping"
                    />
                  )}
                </g>
              );
      ...
      {/* Map Legend */}
      <div className="w-full mt-4 flex items-center justify-between bg-brand-light/40 border border-brand-light/60 px-4 py-2.5 rounded-2xl shrink-0">
        <span className="text-[10px] text-brand-textSecondary font-bold uppercase tracking-wider">Legenda Kepadatan:</span>
        <div className="flex items-center gap-1.5 text-[9px] font-bold text-brand-textSecondary">
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded bg-[#E2ECE5] border border-brand-light" />
            <span>0 kg</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded bg-[#C2E5D0]" />
            <span>Rendah</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded bg-[#8CD1A4]" />
            <span>Sedang</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded bg-[#4CAD73]" />
            <span>Tinggi</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded bg-[#20633C]" />
